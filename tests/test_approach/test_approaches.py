...
from metric.metrics import EuclidianMetric
from resultrecycler import Fixed, NearestNeighbor, FirstDerivative, SecondDerivative, AffineHull, RawData
from resultrecycler.config import Config


class TestApproach(TestCase):
    @classmethod
    def init_data(cls):
        scalar_raw_data_base = [RawData(1, 2, 3, 4)]
        scalar_raw_data_ext = [RawData(1.4, 2.2, 3.6, 4.8)]
        cls.scalar_config = Config(scalar_raw_data_base[0])
        cls.scalar_data_1 = [cls.scalar_config.data_class(raw) for raw in scalar_raw_data_base]
        cls.scalar_data_2 = [cls.scalar_config.data_class(raw) for raw in scalar_raw_data_ext]
        ...
        vector_raw_data_base = [RawData([1, 1], [2, 2],
                                                [[2, 3], [4, 5]],
                                                [[[4, 5], [5, 6]], [[6, 7], [7, 8]]])]
        vector_raw_data_ext = [RawData([1.25, 2.25], [3, -1],
                                                     [[2.2, 3.4], [-1.5, -2.1]],
                                                     [[[0.3, -1.1], [-1.1, 2.4]], [[5, 3], [3, 2]]])]
        cls.vector_config = Config(vector_raw_data_base[0])
        cls.vector_data_1 = [cls.vector_config.data_class(raw) for raw in vector_raw_data_base]
        cls.vector_data_2 = [cls.vector_config.data_class(raw) for raw in vector_raw_data_ext]
    ...
    def setUp(self):
        self.scalar_approach = self.approach_class(self.scalar_config)
        self.vector_approach = self.approach_class(self.vector_config)
...
class ApproachTestSuite(TestSuite):
    def __init__(self, single_test=None):
        ...
        class CurrentFixed(Fixed):
            fixed_value = 7

        # TODO one more 2D-test for meaningful affine hull would be nice
        approach_tests = [single_test] if single_test is not None else [
            {'approach_class': CurrentFixed, 'scalar_result_1': array([7]), 'scalar_result_2': array([7]),
             'vector_result_1': array([7, 7]), 'vector_result_2': array([7, 7])},
            {'approach_class': NearestNeighbor, 'scalar_result_1': array([2]), 'scalar_result_2': array([2.2]),
             'vector_result_1': array([2, 2]), 'vector_result_2': array([3, -1])},
            {'approach_class': FirstDerivative, 'scalar_result_1': array([3.5]), 'scalar_result_2': array([2.56]),
             'vector_result_1': array([6, 9]), 'vector_result_2': array([2.7, -0.85])},
            {'approach_class': SecondDerivative, 'scalar_result_1': array([4]), 'scalar_result_2': array([2.584]),
             'vector_result_1': array([12, 17.25]), 'vector_result_2': array([2.853125, -0.81875])},
            {'approach_class': AffineHull, 'scalar_result_1': array([2]), 'scalar_result_2': array([2.25]),
             'vector_result_1': array([2, 2]), 'vector_result_2': array([3, -1])},
            {'approach_class': WeightedSumApproach, 'scalar_result_1': array([2]), 'scalar_result_2': array([2.166667]),
